...
const Icons = {
  ...
  Gem: () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 3h12l4 6-10 13L2 9z" /><path d="M2 9h20" /><path d="M12 22L6 3" /><path d="M12 22l6-19" /></svg>,
  ...
  BookOpen: () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z" /><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z" /></svg>,
...
export default function EntityInspectorPanel({
  entity, characters, entities, activeChapter, projectPath, projectConfig, calConfig, chapters, onEntityUpdated, onConfigUpdate, initialTab, onNavigateToMark, onReloadCurrentChapter, onFlushEditorSave
}) {
  ...
  const loreData = !groupData ? currEntity : null

  const loreCategories = (() => {
    const cats = new Set(['item'])
  ...
  }, [entity?.id, entity?.type, projectPath, viewMode, filterCharacterId, activeChapter?.chapter_number, activeChapter?.world_time])
  ...
  const typeLabel = entity.category || t('inspector.typeLoreEntity', 'Lore Entity')
    ...
    try {
      ...
      if (groupData) {
        await window.api.updateGroup({
          project_path: projectPath, group_id: currEntity.id, name: editData.name, group_type: editData.group_type,
          description: editData.description, surface_agenda: editData.surface_agenda, true_agenda: editData.true_agenda,
          notes: editData.notes, aliases: aliasArray
        })
      } else if (loreData) {
        ...
        await window.api.updateLoreEntity({
          project_path: projectPath, entity_id: currEntity.id, name: editData.name, category: finalCategory,
          classification: editData.classification, description: editData.description, rules: editData.rules,
          limitations: editData.limitations, origin: editData.origin, notes: editData.notes, aliases: aliasArray
        })
        if (editData.category === 'new_category' && finalCategory) {
          const existingCats = Array.isArray(projectConfig?.lore_categories) ? projectConfig.lore_categories : (typeof projectConfig?.lore_categories === 'string' ? JSON.parse(projectConfig.lore_categories) : [])
  ...
  const handleAddFact = async () => {
    ...
    try {
      await window.api.createKnowledge({
        project_path: projectPath, fact: newFact.fact, learned_in_chapter: activeChapter?.id || null,
        is_secret: newFact.is_secret, character_id: parseInt(newFact.character_id), source_entity_type: entity.type, source_entity_id: entity.id
      })
  ...
  const handleConfirmDeleteEntity = async () => {
    ...
    try {
      if (groupData) await window.api.deleteGroup({ project_path: projectPath, group_id: entity.id })
      else await window.api.deleteLoreEntity({ project_path: projectPath, entity_id: entity.id })
    ...
    <div>
      ...
      <div className="entity-header">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div className={`entity-type-badge ${entity.type}`}><TypeIcon /> {typeLabel}</div>
          {!editMode ? (
      ...
      {activeTab === 'overview' && (
        <>
          <div className="entity-section">
            ...
            {editMode ? renderEditField('', 'description', true) : <div className="entity-bio\">{(loreData || groupData).description || t('inspector.noDescription', 'No description yet.')}</div>}
          </div>
          ...
          {groupData && (
            <div className="entity-section">
              ...
              {editMode ? (
                ...
                <>
                  {groupData.surface_agenda && <div className="entity-agenda-row"><div className="entity-agenda-icon" style={{ color: 'var(--accent-green)' }}><Icons.Eye /></div><div><div className="entity-agenda-label">{t('inspector.surfaceAgenda', 'Surface Agenda')}</div><div className="entity-agenda-text">{groupData.surface_agenda}</div></div></div>}
                  {groupData.true_agenda && <div className="entity-agenda-row"><div className="entity-agenda-icon" style={{ color: 'var(--accent-red)' }}><Icons.EyeOff /></div><div><div className="entity-agenda-label">{t('inspector.trueAgenda', 'True Agenda (Author Only)')}</div><div className="entity-agenda-text">{groupData.true_agenda}</div></div></div>}
                </>
              )}
            </div>
          )}
          ...
          {loreData && (
            <div className="entity-section">
              <div className="entity-section-title"><Icons.Target /> {t('inspector.loreDetails', 'Lore Details')}</div>
              {editMode ? (
                <>
                  <div className="entity-edit-field">
                    <label className="entity-edit-label">{t('inspector.categoryLabel', 'Category')}</label>
                    <select className="entity-edit-input" value={editData.category || ''} onChange={(e) => handleEditField('category', e.target.value)}>
                      <option value="">{t('inspector.selectCategory', 'Select category...')}</option>
                      {loreCategories.map((cat) => <option key={cat} value={cat}>{cat.charAt(0).toUpperCase() + cat.slice(1)}</option>)}
                      <option value="new_category">{t('inspector.newCategoryOption', '+ New Category...')}</option>
                    </select>
                    {editData.category === 'new_category' && <input className="entity-edit-input" style={{ marginTop: '6px' }} type="text" value={editData.customCategory || ''} onChange={(e) => handleEditField('customCategory', e.target.value)} placeholder={t('inspector.newCategoryPlaceholder', 'Type new category...')} autoFocus />}
                  </div>
                ...
                <>
                  {loreData.origin && <div className="entity-detail-row"><div className="entity-detail-label">{t('inspector.origin', 'Origin')}</div><div className="entity-detail-value">{loreData.origin}</div></div>}
                  {loreData.rules && <div className="entity-detail-row"><div className="entity-detail-label">{t('inspector.rules', 'Rules')}</div><div className="entity-detail-value" style={{ whiteSpace: 'pre-wrap' }}>{loreData.rules}</div></div>}
                  {loreData.limitations && <div className="entity-detail-row"><div className="entity-detail-label" style={{ color: 'var(--accent-red)' }}>{t('inspector.limitations', 'Limitations')}</div><div className="entity-detail-value" style={{ whiteSpace: 'pre-wrap' }}>{loreData.limitations}</div></div>}
                </>
              )}
            </div>
          )}
          ...
          <div className="entity-section">
            ...
            {editMode ? renderEditField('', 'notes', true) : <div className="entity-narrative-note">{(loreData || groupData).notes || t('inspector.noNotes', 'No notes.')}</div>}
          </div>
        </>
      )}
